...
class BEiTTransform(object):
    ...
    def __init__(self, cfg):
        ...
        assert patch_size * window_size == cfg.input_size

        self.common_transform = transforms.Compose([
            transforms.ColorJitter(0.4, 0.4, 0.4),
            transforms.RandomHorizontalFlip(p=0.5),
            RandomResizedCropAndInterpolation(size=cfg.input_size, interpolation=cfg.train_interpolation)
        ])
    ...
    def __call__(self, image):
        transformed_image = self.common_transform(image)
        return (self.patch_transform(transformed_image),
                self.visual_token_transform(transformed_image),
