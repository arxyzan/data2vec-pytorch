...
class Data2VecTextEncoder(nn.Module):
    ...
    This module comprises two main modules. The main encoder model e.g, RoBERTa as the student and an EMA instance
        of the same student model as the teacher.

    Args:
        cfg: Config object loaded from a yaml file using OmegaConf. keys and values can be accessed as attributes.
        encoder (FairseqEncoder): Encoder model
    """
    ...
    def _build_teacher(self):
        ...
        if self.cfg.ema_transformer_layers_only:
            for k, _ in self.encoder.embed_positions.named_parameters():
                skip_keys.add(f"embed_tokens.{k}")
            for k, _ in self.encoder.embed_positions.named_parameters():
                skip_keys.add(f"embed_positions.{k}")
            if self.encoder.layernorm_embedding is not None:
                for k, _ in self.encoder.layernorm_embedding.named_parameters():
        ...
        ema = EMA(self.encoder, self.cfg, self.device, skip_keys=skip_keys)
        return ema
    ...
    def set_num_updates(self, num_updates):
        ...
        if self.teacher.decay < 1:
            self.teacher.step(self.encoder)
    ...
    def forward(self, src_tokens, target_tokens=None, features_only=False, return_all_hiddens=False):
        ...
        result = {
            "losses": {
                "main": loss.sum() / math.sqrt(x.size(-1)) if self.loss_scale <= 0 else loss.sum() * self.loss_scale
            },
